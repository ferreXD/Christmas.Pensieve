...
const initializeKeyframes = (initializationConfiguration) => {
     // Try to find an existing <style> tag reserved for Khoshnus
    let style =
        document.querySelector('style[data-khoshnus]') ||
        document.querySelector('style');

    // If none exists, create one and append to <head>
    if (!style) {
    ...
    const drawCss = initializeDrawKeyframesCss({
        svgId: initializationConfiguration.svgId,
        start: initializationConfiguration.start,
        end: initializationConfiguration.end,
    });

    const eraseCss = initializeEraseKeyframesCss({
        svgId: initializationConfiguration.svgId,
        start: initializationConfiguration.start,
        end: initializationConfiguration.end,
    });

    style.innerHTML = (style.innerHTML || '') + drawCss + eraseCss;
}
